...
export function ContactForm({ messages }: ContactFormProps) {
  ...
  async function submitContactForm(form: HTMLFormElement) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          typeof dataRes.error === "string" ? dataRes.error : "Send failed"
        );
      }
      setStatus("success");
      form.reset();
      ...
      setFieldErrors({
        form:
          err instanceof Error ? err.message : messages.errorMessage,
      });
    }
  }
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        void submitContactForm(e.currentTarget);
      }}
